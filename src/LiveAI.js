...
export function setupPong() {
    const canvas = document.getElementById('pong_AI');
    if (!canvas)
        return;
    // --- Evitar doble inicialización ---
    if (canvas._pongAI3dBound)
        return;
    canvas._pongAI3dBound = true;
    // ===== Config & session =====
    const BOT_USER_ID = 0;
    ...
    function applyCanvasFullscreenStyle(active) {
        if (active) {
    ...
    for (let x = -fieldW / 2; x <= fieldW / 2; x += 2) {
        ...
        l.color = new BABYLON.Color3(0.3, 1, 0.4);
        l.parent = gridParent;
    }
    ...
    const nameL = new TextBlock('nameL', p1Nick);
    nameL.color = '#9cff9c';
    nameL.fontSize = 18;
    nameL.left = '-30%';
    nameL.top = '-48%';
    gui.addControl(nameL);
    const nameR = new TextBlock('nameR', aiNick);
    nameR.color = '#9cd3ff';
    nameR.fontSize = 18;
    nameR.left = '30%';
    nameR.top = '-48%';
    gui.addControl(nameR);
    const scoreP = new TextBlock('scoreP', '0');
    scoreP.color = 'white';
    scoreP.fontSize = 44;
    scoreP.left = '-30%';
    scoreP.top = '-42%';
    gui.addControl(scoreP);
    const scoreAI = new TextBlock('scoreAI', '0');
    scoreAI.color = 'white';
    scoreAI.fontSize = 44;
    scoreAI.left = '30%';
    scoreAI.top = '-42%';
    gui.addControl(scoreAI);
    const banner = new TextBlock('banner', '');
    banner.color = '#ffaa3bff';
    banner.fontSize = 56;
    banner.outlineColor = '#134d1f';
    banner.outlineWidth = 2;
    ...
    help.thickness = 0;
    help.background = 'rgba(0,0,0,0.35)';
    help.width = '64%';
    help.height = '22%';
    help.top = '30%';
    ...
    const hint = new TextBlock('hint', '[ CLICK TO START — ENTERS FULLSCREEN ]');
    hint.color = 'white';
    hint.fontSize = 22;
    hint.top = '40%';
    gui.addControl(hint);
    ...
    function aiStep(dt) {
        ...
        if (aiVelZ > AI_CTL.maxSpeed)
            aiVelZ = AI_CTL.maxSpeed;
        if (aiVelZ < -AI_CTL.maxSpeed)
            aiVelZ = -AI_CTL.maxSpeed;
    ...
    function resolvePaddleCollision(paddle, leftSide, dt) {
        // Solo si la bola va hacia la pala correspondiente
        if (leftSide && ballVel.x >= 0)
            return false;
        if (!leftSide && ballVel.x <= 0)
            return false;
        ...
        if (leftSide && ball.position.x > targetX + 0.2)
            return false;
        if (!leftSide && ball.position.x < targetX - 0.2)
            return false;
        // AABB en XZ
    ...
    function startRally(withCountdown, initialServe = false) {
        centerBall();
        serve(initialServe);
        if (withCountdown) {
    ...
    function postResultIfNeeded(playerWon) {
        if (postedResult)
            return;
        postedResult = 1;
    ...
    function aiPressDirection(dir) {
        ...
        if (dir > thr) {
            ...
            if (aiKeyState.down) {
                ...
                aiKeyState.down = false;
            }
        }
        else if (dir < -thr) {
            if (!aiKeyState.down) {
                dispatchKeyDown('ArrowDown');
                aiKeyState.down = true;
            }
            if (aiKeyState.up) {
    ...
    const onKeyUp = (e) => { if (e.key in keys)
        keys[e.key] = false; };
    ...
    engine.runRenderLoop(() => {
        ...
        if (state === 'PLAYING') {
            // Jugador (up=z- ; down=z+)
            if (keys.s)
                p1.position.z = clampZ(p1.position.z - paddleSpeed * dt);
            if (keys.w)
                p1.position.z = clampZ(p1.position.z + paddleSpeed * dt);
            ...
            for (let s = 0; s < steps && state === 'PLAYING'; s++) {
                ...
                if (collideCooldown <= 0) {
                    if (resolvePaddleCollision(p1, true, subDt)) {
                        playerHits += 1;
                    }
                    else if (resolvePaddleCollision(ai, false, subDt)) {
                        aiHits += 1;
                    }
                }
                if (checkGoal())
