...
import { setupPong } from './LiveAI.js'; // ⬅️ import del juego 3D
...
export async function renderPlayAI(root: HTMLElement) 
{
  ...
  let selected: Diff | null = null; /* state management: variable called 'selected' holds the
  ...
  * card or whenever the selection needs to change.*/
  ...
  function updateCardAppearance(c: HTMLButtonElement)
  {
      const on = c.dataset.diff === selected;
      c.classList.toggle('ring-4', on);
      c.classList.toggle('ring-emerald-400', on);
